...
[sco.] train_iou50_mAP: {train_iou50_mAP}, train_iou50_mR: {train_iou50_mR}
[sco.] train_iou_max_recall_0.25: {train_iou_max_recall_25}, train_iou_max_recall_0.5: {train_iou_max_recall_5}
...
[train] train_iou50_mAP: {train_iou50_mAP}, train_iou50_mR: {train_iou50_mR}
[train] train_max_iou25_mR: {train_max_iou25_mR}, train_max_iou50_mR: {train_max_iou50_mR}
...
[val]   val_iou50_mAP: {val_iou50_mAP}, val_iou50_mR: {val_iou50_mR}
[val]   val_max_iou25_mR: {val_max_iou25_mR}, val_max_iou50_mR: {val_max_iou50_mR}
"""
...
[sco.] iou50_mAP: {iou50_mAP}, iou50_mR: {iou50_mR}
"""
...
class Solver():
    ...
    detection=True, reference=True, use_lang_classifier=True, lr_args=None, bn_args=None):
        ...
        self.best = {
            ...
            "iou50_mAP": -float("inf"),
            "iou50_mR": -float("inf"),
            "max_iou25_mR": -float("inf"),
            "max_iou50_mR": -float("inf")
        }
    ...
    def _eval(self, data_dict, refresh=False):
        data_dict = get_eval(
            data_dict=data_dict,
            config=self.config,
        ...
        # print(data_dict["mAP_0.5"], data_dict["mR_0.5"])
        self._running_log["iou50_mAP"] = np.mean(data_dict["mAP_0.5"])
        self._running_log["iou50_mR"] = np.mean(data_dict["mR_0.5"])
        self._running_log["mAP50_metrics"] = data_dict["mAP50_metrics"]
        self._running_log["max_iou25_mR"] = np.mean(data_dict["related_object_max_iou25_mR"])
        self._running_log["max_iou50_mR"] = np.mean(data_dict["related_object_max_iou50_mR"])
    ...
    def _feed(self, dataloader, phase, epoch_id):
        ...
        for it, data_dict in enumerate(dataloader):
            refresh_eval = (it==len(dataloader)-1)  # refresh eval queue
            if_output_train = self._global_iter_id % self.verbose == 0
            if_val = self._global_iter_id % self.val_step == 0 # and self._global_iter_id != 0
            if_output_tensorboard = self._global_iter_id % 50 == 0
            if phase == 'train' and (if_output_train or if_val):
                refresh_eval = True  # must refresh
            ...
            # eval
            start = time.time()
            self._eval(data_dict, refresh_eval)
            ...
            self.log[phase]["iou50_mAP"] = self._running_log["iou50_mAP"]  # calculate once
            self.log[phase]["iou50_mR"] = self._running_log["iou50_mR"]  # calculate once
            self.log[phase]["mAP50_metrics"] = self._running_log["mAP50_metrics"]  # calculate once
            self.log[phase]["max_iou25_mR"].append(self._running_log["max_iou25_mR"])
            self.log[phase]["max_iou50_mR"].append(self._running_log["max_iou50_mR"])
            ...
            if phase == "train":
                ...
                # evaluation
                if if_val: # and self._global_iter_id != 0:
                    print("evaluating...")
        ...
        if phase == "val":
            ...
            if cur_best + cur_best_25 > self.best[cur_criterion] + self.best[cur_criterion_25]:
                ...
                self.best["iou50_mR"] = np.mean(self.log[phase]["iou50_mR"])
            ...
            det_cur_criterion = "max_iou50_mR"
            det_cur_best = np.mean(self.log[phase][det_cur_criterion])
            if det_cur_best > self.best[det_cur_criterion]:
                self.best["max_iou25_mR"] = np.mean(self.log[phase]["max_iou25_mR"])
                self.best["max_iou50_mR"] = np.mean(self.log[phase]["max_iou50_mR"])
                model_root = os.path.join(CONF.PATH.OUTPUT, self.stamp)
                #torch.save(self.model.state_dict(), os.path.join(model_root, "model_last.pth"))
    ...
    def _dump_log(self, phase):
        log = {
            ...
            "score": ["lang_acc", "vqa_acc", "number_acc", "color_acc", "yes_no_acc", "other_acc", "obj_acc", "pos_ratio", "neg_ratio", "iou50_mAP", "iou50_mR", "max_iou25_mR", "max_iou50_mR"]
        }
        for key in log:
            for item in log[key]:
                value = np.mean([v for v in self.log[phase][item]]) if isinstance(self.log[phase][item], Iterable) else self.log[phase][item]
    ...
    def _train_report(self, epoch_id):
        ...
        iter_report = self.__iter_report_template.format(
            ...
            # train_iou50_mR=round(np.mean([v for v in self.log["train"]["iou50_mR"]]), 5),
            train_iou50_mAP=round(self.log["train"]["iou50_mAP"], 5),
            train_iou50_mR=round(self.log["train"]["iou50_mR"], 5),
            train_iou_max_recall_25=round(np.mean([v for v in self.log["train"]["max_iou25_mR"]]), 5),
            train_iou_max_recall_5=round(np.mean([v for v in self.log["train"]["max_iou50_mR"]]), 5),
    ...
    def _epoch_report(self, epoch_id):
        ...
        self._log_eval("grounded object detection results: (validation set):" +
                        json.dumps(self.log["val"]["mAP50_metrics"], indent=4), output=True)
        epoch_report = self.__epoch_report_template.format(
            ...
            train_iou50_mAP=round(self.log["train"]["iou50_mAP"], 5),
            train_iou50_mR=round(self.log["train"]["iou50_mR"], 5),
            train_max_iou25_mR=round(np.mean([v for v in self.log["train"]["max_iou25_mR"]]), 5),
            train_max_iou50_mR=round(np.mean([v for v in self.log["train"]["max_iou50_mR"]]), 5),
            ...
            val_iou50_mAP=round(self.log["val"]["iou50_mAP"], 5),
            val_iou50_mR=round(self.log["val"]["iou50_mR"], 5),
            # val_iou50_mAP=round(np.mean([v for v in self.log["val"]["iou50_mAP"]]), 5),
            # val_iou50_mR=round(np.mean([v for v in self.log["val"]["iou50_mR"]]), 5),
            val_max_iou25_mR=round(np.mean([v for v in self.log["val"]["max_iou25_mR"]]), 5),
            val_max_iou50_mR=round(np.mean([v for v in self.log["val"]["max_iou50_mR"]]), 5),
        )
    ...
    def _best_report(self):
        ...
        best_report = self.__best_report_template.format(
            ...
            iou50_mR=round(self.best["iou50_mR"], 5),
        )
